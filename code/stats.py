...
def cohen_d(orig_accuracy_list, accuracy_list):
    ...
    dof = nx + ny - 2
    pooled_std = np.sqrt(((nx-1)*np.std(orig_accuracy_list, ddof=1) ** 2 + (ny-1)*np.std(accuracy_list, ddof=1) ** 2) / dof) + 1e-6
    result = (np.mean(accuracy_list) - np.mean(orig_accuracy_list)) / pooled_std
    print(result)
    return result
...
def p_value_glm(orig_accuracy_list, accuracy_list):
    orgStd = np.var(orig_accuracy_list, ddof=1)
    accStd = np.var(accuracy_list, ddof=1)

    if(abs(orgStd)< 1e-8 and abs(accStd) <1e-8):
        if(np.average(orig_accuracy_list) < np.average(accuracy_list)):
            p_value_g = 0.001

        elif(abs(orig_accuracy_list[1]-accuracy_list[1])<0.05):
            p_value_g = 0.5
